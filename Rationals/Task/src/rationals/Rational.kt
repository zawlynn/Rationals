...
class Rational(val n: BigInteger, val d: BigInteger) : Comparable<Rational> {
    ...
    operator fun plus(r: Rational): Rational {
        return (n.times(r.d).plus(r.n.times(d))).divBy(r.d.times(d))
    }

    operator fun minus(r: Rational): Rational {
        return (n.times(r.d).minus(r.n.times(d))).divBy(r.d.times(d))
    }
    ...
    operator fun times(r: Rational): Rational {
        return n.times(r.n).divBy(r.d.times(d))
    }

    operator fun div(r: Rational): Rational {
        return n.times(r.d).divBy(d.times(r.n))
    }
...
fun hcf(n1: BigInteger, n2: BigInteger): BigInteger {
    return if (n2 != 0.toBigInteger()) hcf(n2, n1 % n2) else n1
}
...
infix fun BigInteger.divBy(r2: BigInteger): Rational {
    return Rational(this, r2)
}

infix fun Long.divBy(r2: Long): Rational {
    return Rational(toBigInteger(), r2.toBigInteger())
}
